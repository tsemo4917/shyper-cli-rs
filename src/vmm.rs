...
pub fn vmm_boot(vm_id: u32) {
    unsafe {
        if shyper_ioctl!(0x0102, vm_id) != 0 {
...
pub fn vmm_remove(vm_id: u32) {
    unsafe {
        if shyper_ioctl!(0x0110, vm_id) != 0 {
            error!("err: ioctl fail!");
        }
    }
}
...
pub fn vmm_getvmid() {
    unsafe {
        let mut id: u32 = 0;
        if shyper_ioctl!(0x0108, &mut id as *mut u32) != 0 {
            error!("err: ioctl fail!");
        } else {
...
pub fn vmm_list_vm_info() {
    ...
    unsafe {
        let buf = [0; VM_PAGE_SIZE];
        if shyper_ioctl!(0x0100, buf.as_ptr()) != 0 {
            error!("err: ioctl fail!");
            return;
        } else {
            vm_info_list = mem::transmute::<[u8; VM_INFO_LIST_SIZE], VMInfoList>(
                buf[0..VM_INFO_LIST_SIZE].try_into().unwrap(),
            );
        }
    }
